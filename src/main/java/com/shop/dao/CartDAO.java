...
public class CartDAO {
	public List<Cart> getCart(String user) {
		...
		//Get user id object
		Query<User> userid = session.createQuery("FROM User WHERE user = :user");
		userid.setParameter("user", user);	
	...
	public void addNewItemToCart(String user, String item) {
		...
		//Get product obj 
		Query<Product> productid = session.createQuery("FROM Product WHERE product = :item");
