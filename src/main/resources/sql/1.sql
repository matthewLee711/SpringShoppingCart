...
CREATE TABLE user_table (
  ...
  user_type_fk INTEGER REFERENCES user_type_table(type_id)
);

CREATE TABLE shopping_cart_table(
  cart_id serial PRIMARY KEY NOT NULL,
  item_fk INTEGER REFERENCES product_table(product_id),
  number_to_purchase INTEGER,
  buyer_fk INTEGER REFERENCES user_table(user_id)
);
